...
class Play:
    ...
    def to_csv(self):
        ...
        export_df = pd.DataFrame(columns=['id', 'speaker', 'speech', 'pywsd_output', 'tokens_text', 'tokens_emotions', 'scene', 'primary_emotion', 'secondary_emotion'])

        # Loops through each character
        for c in self.characters:
            # Loops through each speech
            for s in c.speeches:
                export_df = export_df.append({'id':s.id, 'speaker':c.name, 'speech':s.text, 'pywsd_output':s.pywsd_output, 'tokens_text':s.tokenized_text, 'tokens_emotions':s.tokenized_emotions, 'scene':s.scene, 'primary_emotion':s.primary_emotion, 'secondary_emotion':s.secondary_emotion}, ignore_index=True)
